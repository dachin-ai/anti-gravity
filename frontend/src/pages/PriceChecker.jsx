...
import { Tabs, Card, Button, Input, InputNumber, Row, Col, Typography, Table, Upload, message, Spin, Alert, Statistic, Divider } from 'antd';
import { InboxOutlined, SyncOutlined, CloudDownloadOutlined, CheckCircleOutlined, CloseCircleOutlined, FileExcelOutlined } from '@ant-design/icons';
import api from '../api';
...
const PriceChecker = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            message.success("Batch calculation complete! Please check the overview below.");
        } catch (error) {
    ...
    const handleDownloadResult = () => {
        if (!batchOverview || !batchOverview.file_base64) return;
        const byteString = atob(batchOverview.file_base64);
    ...
    const previewColumns = batchOverview && batchOverview.preview.length > 0 
        ? Object.keys(batchOverview.preview[0]).map(k => ({
            title: k,
            dataIndex: k,
            key: k,
            render: (text) => {
               if (k === "Gap Warning" && text === "Invalid") return <Text type="danger" strong>Invalid</Text>;
               if (k === "Gap Warning" && Number(text) < 0) return <Text type="danger">{text}</Text>;
               return text;
            }
          }))
        : [];
    ...
    return (
        <div style={{ padding: 24 }} className="glass-panel">
            ...
            <Alert 
                ...
                description="Supports 3 Methods: Listing Method (Merge ID), SKU Method (File Batch), & Direct Input (Real-time). Robust calculation." 
                type="info" 
                showIcon 
                style={{ marginBottom: 24, borderRadius: 12, border: '1px solid var(--primary)', color: 'var(--text-main)' }} 
            />
                    ...
                    <>
                        ...
                        {batchOverview && (
                            <div style={{ marginTop: 32 }}>
                                ...
                                <Title level={4} style={{ fontFamily: "'Outfit', sans-serif" }}>📑 Processing Overview</Title>
                                
                                <Row gutter={16} style={{ marginBottom: 24 }}>
                                    <Col span={8}>
                                        <Card size="small" className="glass-card" style={{textAlign: 'center'}}>
                                            <Statistic title={<Text strong>Total Rows Checked</Text>} value={batchOverview.summary.total} prefix={<SyncOutlined />} valueStyle={{ color: 'var(--primary)' }} />
                                        </Card>
                                    </Col>
                                    <Col span={8}>
                                        <Card size="small" className="glass-card" style={{textAlign: 'center'}}>
                                            <Statistic title={<Text strong>Valid Items</Text>} value={batchOverview.summary.valid} prefix={<CheckCircleOutlined />} valueStyle={{ color: '#10b981' }} />
                                        </Card>
                                    </Col>
                                    <Col span={8}>
                                        <Card size="small" className="glass-card" style={{textAlign: 'center'}}>
                                            <Statistic title={<Text strong>Invalid Items</Text>} value={batchOverview.summary.invalid} prefix={<CloseCircleOutlined />} valueStyle={{ color: '#ef4444' }} />
                                        </Card>
                                    </Col>
                                </Row>

                                <Title level={5} style={{ fontFamily: "'Outfit', sans-serif", color: 'var(--text-muted)' }}>Top 10 Data Preview</Title>
                                <div className="glass-card" style={{ overflow: 'hidden', padding: 8, marginBottom: 24 }}>
                                    <Table 
                            ...
                            <Col span={16}>
                                <div>
                                    ...
                                    <Input 
                                        ...
                                        style={{ marginTop: 8, borderRadius: 8, background: 'rgba(255,255,255,0.7)' }}
                                    />
                                </div>
                            </Col>
                            <Col span={8}>
                                <div>
                                    ...
                                    <InputNumber 
                                        size="large"
                                        style={{ width: '100%', marginTop: 8, borderRadius: 8, background: 'rgba(255,255,255,0.7)' }} 
                                        min={0} 
                                        step={1000} 
                                        value={targetPrice} 
